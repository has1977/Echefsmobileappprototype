...
export function BranchSelectionPage() {
  ...
  console.log('🏢 BranchSelection - Branches from context:', branches);
  console.log('🏢 BranchSelection - Number of branches:', branches?.length);
  console.log('🌍 BranchSelection - Current language:', currentLanguage);

  // If no branches, show a helpful message with link to fix
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-purple-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-3">No Branches Found</h1>
          <p className="text-gray-600 mb-8">
            The database appears to be empty. Click below to fix this issue.
          </p>
          
          <button
            onClick={() => navigate('/quick-db-fix')}
            className="w-full bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-700 hover:to-purple-900 text-white font-bold py-4 px-8 rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105 mb-3"
          >
            🔧 Fix Database Now
          </button>
          ...
          <button
            ...
            className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-6 rounded-xl transition-all mb-3"
          >
            🔄 Reload Page
          </button>
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            ...
            <div className="text-xs text-left bg-gray-50 rounded-lg p-4 space-y-1 font-mono">
              <div>Branches: {branches?.length || 0}</div>
              <div>Language: {currentLanguage}</div>
              <div>localStorage: {localStorage.getItem('echefs_db') ? 'EXISTS' : 'EMPTY'}</div>
              <div>DB Version: {localStorage.getItem('echefs_db_version') || 'NONE'}</div>
            </div>
          </div>
        </div>
      </div>
